function [runtime,ess,numabove,numbelow] = runnrmix(alg,conjugate,sampled,...
...
runtime = output(1);
f=fopen([outputFilename '.ess.r'],'w');
fprintf(f,'library(coda)\n');
fprintf(f,'ess <- function(f) {\n');
fprintf(f,'  t <- read.table(paste(f,".parameters",sep=""));\n');
fprintf(f,'  a <- mcmc(t[,1:3]);\n');
fprintf(f,'  s <- effectiveSize(a);\n');
fprintf(f,'  write(s,file=paste(f,".ess",sep=""));\n');
fprintf(f,'}\n');
fprintf(f,['ess("' outputFilename '");\n']);
fclose(f)
eval(['!r ' outputFilename '.ess.r']);

ess = load('-ascii',[outputFilename '.ess']);
ess = ess(1);
